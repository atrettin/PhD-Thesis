...
understands all of the options you would normally pass to that class.
If 
...
In addition, in \Class{kaobook} some options 
have modified default values.
...
Implement the \Option{justified} and \Option{margin} options.
To be 
...
simple switches supported by \KOMAScript.
See the \KOMAScript\xspace 
documentation for further information.
\end{kaobox}

The above box is an example of a \Environment{kaobox}, which will be 
discussed more thoroughly in \frefch{mathematics}.
...
In particular, the packages we load are:
...
nevertheless grouped them with the others.
By default, the main text is 
...
is not compatible with \Class{kaobook}.
...
\Command{@plaintitle} and \Command{@plainauthor}, respectively.
The PDF 
...
we remark it here.
If you compile the document with pdflatex, the PDF 
...
and two page styles, \Option{plain} and \Option{fancy}.
The layout 
...
\Option{wide} layout the margins are absent.
In \Option{plain} pages the 
...
change page layout and style for these sections of the book.
The front 
matter uses the \Option{margin} layout and the \Option{plain} page 
style.
In the mainmatter the margins are wide and the headings are 
fancy.
In the appendix the style and the layout do not change; however 
...
the preceding part).
In the backmatter the margins shrink again and we 
