...
\LaTeX\xspace document.
The \Package{graphicx} package is already loaded 
and configured in such a way that the figure width is equal to the 
textwidth and the height is adjusted in order to maintain the original 
aspect ratio.
As you may have imagined, the captions will be 
positioned\ldots well, in the margins.
...
\Command{captsetup} from the \Package{caption} package.
Remember that if 
...
position is handled by the \Package{floatrow} package.
Achieving this 
...
to the margin and the caption is below it.
\reffig{marginmonalisa} is 
...
Notice how you can place the 
caption above the table by just placing the \Command{caption} command 
before beginning the \Environment{tabular} environment.
Usually, figure 
captions are below, while table captions are above.
This rule is also 
...
can insert figures which span the whole page width.
For example, here 
...
necessary, until it is aesthetically pleasing.
...
position it occupies in the source code.
However, this specifier does 
...
margin).
However, sometimes you may need a \Environment{longtable} that 
is so wide that it trespass into the margins; in those cases, you may 
want to also increase the width of the caption.
