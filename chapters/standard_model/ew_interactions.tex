...
The NC part describes the exchange of neutral $Z^0$ bosons, which couples to all quarks and leptons except for right-handed neutrinos.
...
In the case that $Q^2 \ll m_W$ and that the energy is well above the muon production limit, the matrix element is
\begin{equation}
...
The variables $s$ and $u$ are Mandelstam variables that are functions of the momentum transfer and the factors $A$, $B$ and $C$ are functions of the form factors of the nucleon.
...
With the vector form factors and the coupling constant $g_A$ are well constrained from electron scattering and nuclear beta decay, respectively, the only major source of uncertainty on this cross-section comes from that on the axial mass.
For interactions in ice, the majority of nucleons are bound in oxygen nuclei.
This leads to corrections to the cross-section in \refeq{ccqe-xsec} due to nuclear effects that become more important as the energy increases and that are beyond the scope of this discussion.
