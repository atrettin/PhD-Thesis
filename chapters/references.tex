...
use the \Command{sidecite}\index{\Command{sidecite}} command.
It does 
...
tex file.
This package accepts all the options that you can pass to 
\Package{biblatex}, and actually it passes them to \Package{biblatex} 
under the hood.
Moreover, it also defines some commands, like 
...
either \enquote{true} or \enquote{false} as a value.
If you
...
a hyperlink bothers more than one biblatex user.
There are
...
in the margin.
However, this command would be useless without a way to 
customise the format of the citation, so the \Class{kaobook} provides 
also the \Command{formatmargincitation} command.
By \enquote{renewing} 
...
useful when you also change the default bibliography style.
For 
...
and \Command{sdcite} are just examples.
A full
...
external tool, which for this class is biber.
You need to run the 
...
your book.
For instance, I previously defined some glossary entries and 
...
short one \acrshort{fpsLabel}.
These entries will appear in the glossary 
in the backmatter.

Unless you use \href{https://www.overleaf.com}{Overleaf} or some other 
fancy IDE for \LaTeX, you need to run an external command from your 
terminal in order to compile a document with a glossary.
In particular, 
...
\enquote{subject} in the text.
For instance, at the start of this 
paragraph I would write \lstinline|index{index}|, and an entry would be 
added to the Index in the backmatter.
Check it out!
...
this book.
To insert a nomenclature, we use the package \Package{nomencl} and
add the terms with the command \Command{nomenclature}.
We put then a
...
this class.
However, the configuration of the elements is best done in 
...
\Package{scrhack} to load the hack-file for \Package{nomencl}.
When 
...
across the book.
First, you can label each element with a specific 
command.
For instance, should you want to label a chapter, you would put 
\lstinline|\labch{chapter-title}| right after the \Command{chapter} 
directive.
This is just a convenience, because \Command{labch} is
actually just an alias to \lstinline|\label{ch:chapter-title}|, so it 
spares you the writing of \enquote{ch:}.
We defined similar commands for 
...
we provide different commands to reference the same thing.
For instance, 
in some occasions you may want to reference the chapter by name, but 
other times you want to reference it only by number.
In general, there 
are four reference style, which we call plain, vario, name, and full.

The plain style references only by number.
It is accessed, for chapters, 
with \lstinline|\refch{chapter-title}| (for other elements, the syntax 
is analogous).
Such a reference results in: \refch{references}.
...
the name style.
As you can see, the page is referenced in 
\Package{varioref} style.

The full style references everything.
You can use it with 
...
However, not all elements implement all the four styles.
The commands 
...
configured, from the bibliography to the glossary.
...
know how the compilation works in Windows or Mac.
The tips, therefore, 
...
GitHub, do as follows.
\lstinline[language=bash]|cd| into the root 
