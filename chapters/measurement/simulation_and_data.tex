...
\begin{enumerate}
    \item Simulation of particle interactions (sampling of primary particles, production of secondary particles, and photon production)
...
\begin{equation}
    w = \frac{1}{F_{\mathrm{sim}}} = \frac{1}{T_{\mathrm{sim}} \times \Phi_{\mathrm{sim}}}\;,
\end{equation}
where $T_{\mathrm{sim}}$ is the simulated live time and $\Phi_{\mathrm{sim}}$ is the simulated flux per area, solid angle, energy, and time. This weight, when multiplied with the flux of a given physics model and a live time, gives the expected number of events that this simulated event corresponds to.
