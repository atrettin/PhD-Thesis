...
To make a measurement, the discrepancy between the histograms from reweighted MC events and observed data events has to be measured by an appropriate test statistic. This measurement uses a modified $\chi^2$ test statistic defined as
\begin{equation}
...
\begin{table}
    ...
    \begin{tabular}{clSS} \toprule
        ...
        \multirow{4}{5em}{cross-section}& $M_{A}^{CCQE}$ &  0.043 & 0.043 \\
        & $M_{A}^{CCRES}$ & 0.607 & 0.607  \\
        & DIS CSMS & 0.027  & 0.0267 \\ 
        & NC norm & 1.121 &  0.605 \\ \midrule
        \multirow{5}{5em}{detector systematics} & $\epsilon_\mathrm{DOM}$ & 1.065  & 0.647 \\
        & hole ice $p_0$ & -0.267  &  \\
        & hole ice $p_1$ & -0.042  &  \\
        & ice absorption & 0.973  &  \\
        & ice scattering & 0.988 &  \\ \midrule
        \multirow{2}{5em}{norm}& $N_\mu$ & 1.371  &  \\
        & $N_\nu$ & 0.823 &  \\
        \bottomrule
    \end{tabular}
\end{table}

The best-fit point of the hole ice parameters $p_0$ and $p_1$ lies in close proximity to the results of the LED flasher fits shown in \reffig{hole-ice-parametrization}. The resulting light acceptance curve is shown in \reffig{hole-ice-bfp-comparison} compared to the curves produced by \emph{in-situ} LED calibration studies. The result of the three-flavor oscillation fit agrees with the LED calibration result in preferring a lower forward acceptance than the baseline hole ice model.
\begin{figure}
    \centering
