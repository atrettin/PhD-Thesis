...
The framed parameters are (from top to bottom and from left to right in the bottom row): \texttt{barr\_w\_K}, \texttt{barr\_w\_antiK}, \texttt{barr\_g\_Pi}, \texttt{pion\_ratio}, \texttt{barr\_h\_Pi}\todo{ugly parameter names}.
...
The best fit result\todo{calculate 90\%, 99\% exclusion} for the mixing amplitudes is
\begin{align*}
    |U_{\mu 4}|^2  &= 0.00449 \\
    |U_{\tau 4}|^2 &= 0.00307\;.
\end{align*}
...
For both $|U_{\mu 4}|$ and $|U_{\tau 4}|$, the analysis can constrain the amplitude of active-sterile mixing to $<0.1$ at $>99\%$ C.L., setting the strongest limit\todo{put actual limits here} on $|U_{\tau 4}|$ to date.
...
As expected, this effect is stronger when one of the mixing amplitudes is close to zero.\todo{box in contour plot is in awkward spot}
